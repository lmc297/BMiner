...
ui <- fluidPage(
   ...
   sidebarLayout(
     ...
     conditionalPanel(condition = "input.tabs=='Virulence'",
            ...
            choices=c("Virulence Gene Distribution (Bar Plot)"=1, 
     ...
     conditionalPanel(condition = "input.tabs=='panC'",
                      selectInput("pplot",
                                  ...
                                  choices=c("panC Clade Distribution (Bar Plot)"=1),
                                  selected=1)),
     conditionalPanel(condition = "input.tabs=='16S'",
                      selectInput("splot",
                                  label="16S rDNA Typing Analyses",
                                    choices=c("Closest 16S Sequence Distribution (Bar Plot)"=1),
                                  selected=1)),
     conditionalPanel(condition = "input.vplot=='1' && input.tabs=='Virulence' || 
                      input.mplot=='1' && input.tabs=='MLST' || 
                      input.pplot=='1' && input.tabs=='panC' ||
                      input.rplot=='1' && input.tabs=='rpoB' ||
                      input.splot=='1' && input.tabs=='16S'",
                      radioButtons("bcType",label = "Y-axis Display",
                                    choices = list("Counts","Percent (%)"),
                                   selected="Counts")),
     
     conditionalPanel(condition = "input.tabs!='Home' && input.tabs != 'Metadata' && input.vplot!='4'",
     ...
     conditionalPanel(condition = "input.tabs=='Virulence' && input.vplot=='4'",
      ...
      mainPanel(tabsetPanel(id = "tabs",
        ...
        tabPanel("Virulence",
                ...
                fluidRow(column(width=12,h3("Selected sample(s)"),
                                helpText("Click on a point to view sample name(s) and the associated coordinates."),
...
server <- function(input, output) {
  ...
  barchart<-function(vec,xlab,ylab,text.angle,vmat){
    ...
    if (input$bcType=="Counts"){
    bc<-ggplot(myst,aes(x=reorder(myvec,-Freq),y=Freq,fill=reorder(myvec,-Freq)))+geom_bar(stat="identity",show.legend = FALSE)+xlab(label = xlab)+ylab(label = paste("Total Number of Samples",ylab))+theme(axis.text.x = element_text(angle=text.angle))}
    else if (input$tabs!="Virulence"){
      bc<-ggplot(myst,aes(x=reorder(myvec,-Freq),y=(Freq/sum(Freq))*100,fill=reorder(myvec,-Freq)))+geom_bar(stat="identity",show.legend = FALSE)+xlab(label = xlab)+ylab(label = paste("Percentage (%) of Samples",ylab))+theme(axis.text.x = element_text(angle=text.angle))}
    else{
      bc<-ggplot(myst,aes(x=reorder(myvec,-Freq),y=(Freq/nrow(vmat))*100,fill=reorder(myvec,-Freq)))+geom_bar(stat="identity",show.legend = FALSE)+xlab(label = xlab)+ylab(label = "Percentage (%) of Samples with Virulence Gene")+theme(axis.text.x = element_text(angle=text.angle))}
    return(bc)
  }
  ...
  make.vmat<-function(lines){
    # virulence
    isolate.names<-lapply(lines,function(x) strsplit(x[1],split="BTyper Results for ")[[1]][2])
    vgenes<-sapply(lines,function(x) which(vapply(x,function(r) grepl("\\|",r)&&!(grepl("rpoB\\|",r)),FUN.VALUE = 1)==1))
    print("vgenes")
 ...
 # home text
  ...
  #output$virbarchart <- renderPlot({
    ...
    if (is.null(infile)){
      ...
      vmat<-make.vmat(lines=lines)
      if (is.null(vmat)){
      text = paste("\n   There don't appear to be any virulence genes\n",
      ...
      vbar<-barchart(vec=finalvvec,xlab="Virulence Genes", ylab="with Virulence Gene",text.angle=45,vmat=vmat)
      return(vbar)}
    else if (vselect=="2"){
