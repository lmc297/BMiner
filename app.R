  ...
  sidebarLayout(
    sidebarPanel(
      ...
      conditionalPanel(condition = "input.aplot=='3' && input.tabs=='Antimicrobial Resistance'",# && output.listPC",
                       ...
                       uiOutput("amrlistPC2"),
                       uiOutput("amrlistPC3")),
      conditionalPanel(condition = "input.tabs=='ANIb'",
                       selectInput("bplot",
                                   label="ANIb Typing Analyses",
                                   choices=c(
                                     "Closest Species Distribution (Bar Plot)"=1),
                                   selected=1)),
      ...
      conditionalPanel(condition = "input.aplot=='1' && input.tabs=='Antimicrobial Resistance'", 
                       radioButtons("bcTypea",label = "Y-axis Display",
                                    choices = list("Counts","Percent (%)"),
                                    selected="Counts")),
      conditionalPanel(condition = "input.bplot=='1' && input.tabs=='ANIb'", 
                       radioButtons("bcTypeb",label = "Y-axis Display",
                                    choices = list("Counts","Percent (%)"),
                                    selected="Counts")),
    ...
    mainPanel(tabsetPanel(id = "tabs", 
                          ...
                          tabPanel("ANIb",plotOutput("anibarchart")),
...
server <- function(input, output) {
  ...
  barchart<-function(vec,xlab,ylab,text.angle,vmat){
    ...
    if (input$bcTypev=="Counts" && input$tabs=="Virulence"){
      ...
      bc<-ggplot(myst,aes(x=reorder(myvec,-Freq),y=Freq,fill=reorder(myvec,-Freq)))+geom_bar(stat="identity",show.legend = FALSE)+xlab(label = xlab)+ylab(label = paste("Total Number of Genomes",ylab))+theme(axis.text.x = element_text(angle=text.angle))}
    else if (input$bcTypeb=="Counts" && input$tabs=="ANIb"){
      bc<-ggplot(myst,aes(x=reorder(myvec,-Freq),y=Freq,fill=reorder(myvec,-Freq)))+geom_bar(stat="identity",show.legend = FALSE)+xlab(label = xlab)+ylab(label = paste("Total Number of Genomes",ylab))+theme(axis.text.x = element_text(angle=text.angle))}
  ...
  aniB<-function(){
    ...
    if (is.null(infile)){
      ...
      # ANIb
      bgenes<-lapply(lines,function(x) which(x%in%"Most Similar Reference Genome\tANI\tCoverage"))
      bts<-c()
      for (i in 1:length(lines)){
        bgroup<-strsplit(lines[[i]][bgenes[[i]][1]+1],split="\t")[[1]][1:2]
        bunit<-paste(bgroup[1],bgroup[2],sep="_")
        if (!grepl("Most Similar Reference",bunit)){
          bts<-append(bts,bunit)}
      }
      bts<-gsub("_.*","\n",bts)
      bts<-gsub("\\*","",bts)
      testbts<-bts[which(as.character(bts)!="NA\n")]
      print(testbts)
      if (length(testbts)>0){
        bbar<-barchart(vec=bts,xlab="Reference Genome", ylab="",text.angle=45,vmat=NULL)}
      else{
        text<-paste("No ANIb results were detected","in the selected files.",sep="\n")
        bbar<-empty.plot(text = text)}
      return(bbar)}
